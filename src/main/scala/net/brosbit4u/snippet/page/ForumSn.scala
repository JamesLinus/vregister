...
class ForumSn extends UsersOperations {
  ...
  def showThreads(node:NodeSeq):NodeSeq = {
    ...
    <h2><span> {if (id.is == 0) "Najnowsze wpisy" else department.get.name.is}</span></h2>
      <table><tr class="header1"><td class="col1"> </td> <td class="col2">Wątek / Autor wątku</td>
          <td class="col3">Posty:</td><td class="col4">Ostatni post / autor</td></tr>
        {for (t <- threads) yield { nr += 1
            <tr class={if (nr % 2 == 0) "row1" else "row2"} >
        <td >{if(isAdmin) <a href={"/forum?id=-1&t=" + t.id.toString} onclick="return confirm('Na pewno usunąć wątek?');" ><img src={"style/images/delpost.png"} /> </a>
                          else <a href={"/forumpost?id=" + t.id.is.toString}><img src="style/images/post.png" /></a>  }</td>
        <td ><a href={"/forumpost?id=" + t.id.is.toString}>{t.title.is}</a>
        <br /> Autor: {t.author.obj.get.getFullName}</td>
