...
def departments(node:NodeSeq):NodeSeq = {
  ...
  def showThread() = {
    ...
    S.param("dp").getOrElse("0") match {
      ...
      case idCom => {
            ForumCom.find(idCom) match {
              case Full(comment) => { S.notice("Znaleziono komentarz!")
                  if (isAdmin ) {
                    comment.delete_!
    ...
    var nr = 0
    val n = <div class="forum"><h2><span>{post.department.obj.get.name}</span></h2>
      <table><tr class="header2"><td colspan="4">{post.title}</td></tr>
      {for(com <- coments) yield { nr += 1
            <tr class={if (nr % 2 == 0) "row1" else "row2"}>
            <td><div class="comment">{Unparsed(com.content.get)}<br />_____<br />
      ...
      {if(isAdmin) <a href={"forumpost?id="+id.toString + "&dp="+com.id.is.toString } onclick=" return confirm('Na pewno usunąć post?');" ><img src="style/images/delpost.png" style="float:right;" /> </a> } </div></td></tr> }
      }
      </table>
    </div>
        n
  ...
  def newComment(node:NodeSeq):NodeSeq = {
    if(!isLoged) return <h3>Zaloguj się aby odpowiedzieć  (login i hasło uzyskasz w szkole)</h3>
     var content = ""
